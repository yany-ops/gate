...
export default function EditRolePage() {
  ...
  const [users, setUsers] = useState<string[]>(["dev1@example.com", "dev2@example.com"])
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Edit {roleData.name}</h1>
        </div>
        <AlertDialog>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edit role information and permissions</CardDescription>
        </CardHeader>
        <CardContent>
          <form id="role-details-form" onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="description" defaultValue={roleData.description} rows={2} />
              </div>
                ...
                <div className="border rounded-md p-4 space-y-3">
                  {availableClusters.map((cluster) => (
                    <div key={cluster.id} className="flex items-center space-x-2">
                      <Checkbox id={`cluster-${cluster.id}`} defaultChecked={cluster.selected} />
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        {resources.map((resource) => (
                          <div key={resource.name} className="flex items-center space-x-2">
                            <Checkbox id={`resource-${resource.name}`} defaultChecked={resource.selected} />
                            <Label htmlFor={`resource-${resource.name}`} className="font-normal">
                              {resource.name}
                            </Label>
                          </div>
                        ))}
                      ...
                      <div className="space-y-2">
                        {actions.map((action) => (
                          <div key={action.name} className="flex items-center space-x-2">
                            <Checkbox id={`action-${action.name}`} defaultChecked={action.selected} />
                            <Label htmlFor={`action-${action.name}`} className="font-normal">
                              {action.name}
                            </Label>
                          </div>
                        ))}
        ...
        <CardFooter className="flex justify-end">
          <Button type="submit" form="role-details-form" disabled={isSubmitting}>
